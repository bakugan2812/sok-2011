...
df_gdp <- df_gdp %>% 
  filter(!is.na(iso3c) & iso3c != "") #Vi ønsker å ta vekk land som ikke har en iso3c kode. Dessverre er manglende observasjoner for "iso3c" (landkode) kodet som "blanks" isteden for "missing". Denne koden korrigerer dette.
...
ggplot (df_gdp, aes (x=year, y= gdppc, color = country))+
  geom_line (size = 1)+
  theme_minimal ()
...
df <- subset(no_outliers, select = c("avg_gdpgrowth", "avg_p", "avg_nsy", "avg_educ", "avg_nry", "avg_gi", "avg_n", "avg_gx", "ln_gdppc0"))
# Gi beskrivende navn til variablene (i samme rekkefølge som de ligger i datasettet)
labs <- c("Vekstrate i BNP(%)", "Befolkningsvekst(%)", "Nettosparing", "Humankapital", "Reduksjonsrate i naturressurser", "Vekstrate i investeringer", "Vekstrate i arbeidskraft", "Vekstrate i eksport", "BNP per innbygger i 2000") 
...
plot1 <- ggplot(no_outliers, aes(x = avg_nsy, y = ln_gdppc, na.rm = TRUE)) +
  ...
  ggtitle("Figur 1: Nettosparing") + # Legg til en overskrift på figuren
...
plot2 <- ggplot(no_outliers, aes(x = avg_p, y = ln_gdppc, na.rm = TRUE)) +
  ...
  ylab("BNP per innbygger 2019") + # Beskrivelse for y-akselen
  ggtitle("Figur 2: Befolkningsvekst") + # Legg til en overskrift på figuren
  theme_minimal(base_size = 14, base_family = "Georgia") + # Tekststørrelse og font
  geom_point(aes(size = poptot, color = region), alpha = 0.8) + # Størrelse (farge) på bobblene avhenger befolkningsstørrelse (region)
  scale_x_continuous(labels = dollar)  + # Legg til et dollar tegn til y-akselen
  scale_size_area(guide = "none", max_size = 14) + #Ta vekk legend for befolkningsstørrelse
...
plot3 <- ggplot(no_outliers, aes(x = avg_educ, y = ln_gdppc, na.rm = TRUE)) +
  ...
  ylab("BNP per innbygger 2019") + # Beskrivelse for y-akselen
  ggtitle("Figur 3: Humankapital") + # Legg til en overskrift på figuren
  theme_minimal(base_size = 14, base_family = "Georgia") + # Tekststørrelse og font
  geom_point(aes(size = poptot, color = region), alpha = 0.8) + # Størrelse (farge) på bobblene avhenger befolkningsstørrelse (region)
  scale_x_continuous(labels = dollar)  + # Legg til et dollar tegn til y-akselen
  scale_size_area(guide = "none", max_size = 14) + #Ta vekk legend for befolkningsstørrelse
...
plot4 <- ggplot(no_outliers, aes(x = avg_nsy, y = avg_gdpgrowth, na.rm = TRUE)) +
  xlab("Nettosparing") + # Beskrivelse for x-akselen
  ylab("Gjennomsnittelig vekst i BNP") + # Beskrivelse for y-akselen
  ggtitle("Figur 4: Vekst i BNP og Nettosparing") + # Legg til en overskrift på figuren
  theme_minimal(base_size = 14, base_family = "Georgia") + # Tekststørrelse og font
  geom_point(aes(size = poptot, color = region), alpha = 0.8) + # Størrelse (farge) på bobblene avhenger befolkningsstørrelse (region)
  scale_x_continuous(labels = dollar)  + # Legg til et dollar tegn til y-akselen
  scale_size_area(guide = "none", max_size = 14) + #Ta vekk legend for befolkningsstørrelse
  ...
  scale_colour_manual(values = rainbow(9)) +# Velg farger til bobblene
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_rect(fill = 'white'))+
  scale_y_continuous() + # logaritmere BNP pc og velg hvilke "ticks" som skal vises
  scale_x_continuous() # Velg vilke "ticks" som skal vises på x-akselen
plot4
...
plot5 <- ggplot(no_outliers, aes(x = avg_educ, y = avg_gdpgrowth, na.rm = TRUE)) +
  xlab("Humankapital") + # Beskrivelse for x-akselen
  ylab("Gjennomsnittelig vekst i BNP") + # Beskrivelse for y-akselen
  ggtitle("Figur 5: Vekst i BNP og Humanpaital") + # Legg til en overskrift på figuren
  theme_minimal(base_size = 14, base_family = "Georgia") + # Tekststørrelse og font
  geom_point(aes(size = poptot, color = region), alpha = 0.8) + # Størrelse (farge) på bobblene avhenger befolkningsstørrelse (region)
  scale_x_continuous(labels = dollar)  + # Legg til et dollar tegn til y-akselen
  scale_size_area(guide = "none", max_size = 14) + #Ta vekk legend for befolkningsstørrelse
...
modell2 <- lm(avg_gdpgrowth ~ avg_nsy + avg_educ, data = df_growth2019)
tab_model(modell2)
...
#Humankapital
model1 <- lm(avg_gdpgrowth  ~ avg_educ, data= no_outliers)
tab_model(model1)

#Befolkningsvekst
model1 <- lm(avg_gdpgrowth  ~ avg_p, data= no_outliers)
tab_model(model1)

#Arbeidskraft
model1 <- lm(avg_gdpgrowth  ~ avg_n, data= no_outliers)
tab_model(model1)

#Eksport vekstrate
model1 <- lm(avg_gdpgrowth  ~ avg_gx, data= no_outliers)
tab_model(model1)

#Vekst i innvesteringer
model1 <- lm(avg_gdpgrowth  ~ avg_gi, data= no_outliers)
tab_model(model1)

#Bruk av naturressurser
model1 <- lm(avg_gdpgrowth  ~ avg_nry, data= no_outliers)
tab_model(model1)

#Nettosparing
model1 <- lm(avg_gdpgrowth  ~ avg_nsy, data= no_outliers)
tab_model(model1)

#bnp per innbygger 2000
model1 <- lm(avg_gdpgrowth  ~ ln_gdppc0, data= no_outliers)
tab_model(model1)
...
library(dplyr)
#Samlet modell
model1 <- lm(avg_gdpgrowth ~ avg_educ + avg_p + avg_n + avg_gx + avg_gi + avg_nry + avg_nsy + ln_gdppc0, data = no_outliers)
tab_model(model1)

model1_summary <- summary(model1)
model1_summary_table <- cbind(model1_summary$coefficients[,1:4], F.value = model1_summary$fstatistic[1])
model1_summary_table

kbl(model1_summary_table, align = c("l", rep("r", ncol(model1_summary_table)-1)), 
    caption = "Regresjonsmodell for vekst i BNP") %>%
  kable_styling(full_width = FALSE)

model1_summary <- summary(model1)
model1_summary_table <- cbind(model1_summary$coefficients[,1:4], F.value = model1_summary$fstatistic[1], 
                              R2 = round(model1_summary$r.squared, 3), 
                              Adj.R2 = round(model1_summary$adj.r.squared, 3))
model1_summary_table
...
# Regresjonsanalyse
model1 <- lm(avg_gdpgrowth ~ avg_educ + avg_p + avg_n + avg_gx + avg_gi + avg_nry + avg_nsy + ln_gdppc0, data = df)

# Summerer opp modellen
model1_summary <- summary(model1)

# Henter ut koeffisienter og f-verdi fra summeringsobjektet og legger til R2-verdier
model1_summary_table <- cbind(model1_summary$coefficients[,1:4], 
                              F.value = model1_summary$fstatistic[1],
                              R2 = round(model1_summary$r.squared, 3),
                              `Adj. R2` = round(model1_summary$adj.r.squared, 3))

# Legger til signifikansnivå på f-verdien
p_value <- pf(model1_summary$fstatistic[1], model1_summary$fstatistic[2], model1_summary$fstatistic[3], lower.tail = FALSE)
model1_summary_table <- model1_summary_table %>%
  mutate(F.value = sprintf("%.2f%s", F.value, ifelse(p_value < 0.001, '***', ifelse(p_value < 0.01, '**', ifelse(p_value < 0.05, '*', '')))))

# Gir variablene mer beskrivende navn
colnames(model1_summary_table) <- c("Estimate", "Std. Error", "t value", "Pr(>|t|)", "F.value", "R2", "Adj. R2")
...
modell2 <- lm(avg_gdpgrowth ~ avg_nsy + avg_educ, data = no_outliers)
tab_model(modell2)
